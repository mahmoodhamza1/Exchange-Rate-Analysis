                ...
                sidebarLayout(
                  ...
                  mainPanel(
                    tabsetPanel(
                      ...
                      tabPanel("Time Series Decomposition", plotOutput("decompPlot")),
                      tabPanel("GARCH Volatility", plotOutput("garchVolatilityPlot"))
...
server <- function(input, output) {
  output$timeSeriesPlot <- renderPlot({
    # Ensure that a currency is selected and date range is valid
    req(input$currency, input$dateRange[1], input$dateRange[2])
    
    # Filter data based on user input
    filtered_data <- exchange_rates %>%
      filter(Date >= input$dateRange[1] & Date <= input$dateRange[2]) %>%
      select(Date, .data[[input$currency]]) %>%
      na.omit()
    
    # Check if filtered_data is not empty
    if (nrow(filtered_data) > 0) {
      # Generate the plot
      ggplot(filtered_data, aes(x = Date, y = .data[[input$currency]])) +
      ...
      return(ggplot() +
               ...
               theme_void() +
               labs(title = "No data available for the selected range/currency"))
    }
  })
  ...
  output$decompPlot <- renderPlot({
    ...
    # Check if there are enough data points for STL decomposition
    if (length(exchange_rate_ts) < 2 * frequency(exchange_rate_ts)) {
      return(ggplot() +
               geom_blank(aes(x = 1, y = 1)) +
               theme_void() +
               labs(title = "Not enough data points for STL decomposition"))
    }
    ...
    # Create the plots for each component
  ...
  output$garchVolatilityPlot <- renderPlot({
    ...
    if(nrow(filtered_data) > 30) {  # Ensure there's enough data
      # Fit a GARCH(1,1) model
      spec <- ugarchspec(variance.model = list(model = "sGARCH", garchOrder = c(1, 1)),
                         mean.model = list(armaOrder = c(0, 0), include.mean = TRUE))
      garch_fit <- ugarchfit(spec = spec, data = filtered_data[[2]])
      
      # Extract the conditional volatility from the GARCH model
      volatility <- sigma(garch_fit)
      
      # Create a data frame for the volatility plot
      volatility_data <- data.frame(Date = filtered_data$Date, Volatility = volatility)
      ...
      ggplot() +
        geom_blank(aes(x = 1, y = 1)) +
        theme_void() +
        labs(title = "Not enough data to fit GARCH model")
    }
  })
}
...
tabPanel("Time Series Decomposition", 
         ...
         wellPanel(
           strong("Time Series Decomposition Summary:"),
...
#Reactive expression to perform STL decomposition on filtered data
decomposed <- reactive({
